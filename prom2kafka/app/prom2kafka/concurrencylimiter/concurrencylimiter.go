...
var (
	concurrencyLimitReached = metrics.NewCounter(`prom2kafka_concurrent_insert_limit_reached_total`)
	concurrencyLimitTimeout = metrics.NewCounter(`prom2kafka_concurrent_insert_limit_timeout_total`)

	_ = metrics.NewGauge(`prom2kafka_concurrent_insert_capacity`, func() float64 {
		return float64(cap(ch))
	})
	_ = metrics.NewGauge(`prom2kafka_concurrent_insert_current`, func() float64 {
		return float64(len(ch))
	})
)

// Overloaded returns whether the max concurrent requests are already in use
